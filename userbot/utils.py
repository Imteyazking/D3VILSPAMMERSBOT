...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        sys.modules["d3vilbot.utils"] = userbot.utils
        sys.modules["d3vilbot"] = userbot
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                d3vilreg = "^" + Config.COMMAND_HAND_LER
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                d3vilreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def unsavegif(event, d3vilkrish):
    try:
        await event.client(
